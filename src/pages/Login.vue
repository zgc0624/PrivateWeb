...
function handleError(e: any): string {
  const msg = e.message || ''
  if (msg.includes('EMAIL_EXISTS')) return '该邮箱已注册'
  if (msg.includes('INVALID_PASSWORD') || msg.includes('EMAIL_NOT_FOUND')) return '邮箱或密码错误'
  if (msg.includes('WEAK_PASSWORD')) return '密码太简单，至少6位'
  if (msg.includes('EMAIL_NOT_VERIFIED')) return '请先验证邮箱'
  if (msg) return msg
  return '操作失败，请重试'
}
...
async function handleSubmit() {
  ...
  try {
    if (isLogin.value) {
      ...
      const redirect = (route.query.redirect as string) || '/cloud'
